...
//! - 站点文档根：`/home/{username}/www/{sanitize(name)}-{id}/`，
//!   首次同步自动创建并写占位 index.html
//! - 配置写入后先执行 `nginx -t` 校验，失败即回滚删除文件，绝不带着坏配置 reload
//! - PHP ：`php_socket` 形如 `unix:/path` 或 `host:port`；为 None 时不生成 PHP location
//!
...
fn render_acme_location() -> String {
    ...
    format!(
        "\n    # ACME HTTP-01 Domain Validation (Generated by ZAP , DO NOT EDIT) \n\
...
/// 幂等发布限速 / 限并发的共享 zone（写进 nginx include 目录，位于 http 上下文）。
///
/// zone 与站点 vhost 解耦：所有站点共用 `zap_req_limit` / `zap_conn_limit`，
/// 各自的 rate / burst / 上限写在自己的 `limit_req` / `limit_conn` 指令里
/// （nginx 允许在使用处覆盖 zone 的默认 rate）。无站点引用时残留亦无害。
fn ensure_limit_zones() -> Result<(), String> {
    let content = "# Generated by Zap Panel — 站点安全共享 zone（限速 / 限并发）— DO NOT EDIT\n\
limit_req_zone $binary_remote_addr zone=zap_req_limit:10m rate=1r/s;\n\
limit_conn_zone $binary_remote_addr zone=zap_conn_limit:10m;\n";
    super::webconf::publish_named("nginx", "00-zap-limits.conf", content).map(|_| ())
}
...
fn render_security(sec: Option<&SiteSecuritySpec>, waf_ready: bool) -> String {
    let Some(s) = sec else {
        return String::new();
    };
    ...
    if s.limit_req_enable {
        let rate = s.limit_req_rate.max(1);
        let burst = s.limit_req_burst.max(1);
        out.push_str(&format!(
            "    limit_req zone=zap_req_limit rate={rate}r/s burst={burst} nodelay;\n\
             limit_req_status 429;\n"
        ));
    }
    if s.limit_conn_enable {
        out.push_str(&format!(
            "    limit_conn zap_conn_limit {};\n\
...
fn render_vhost_full(a: VhostRenderSpec<'_>) -> String {
    ...
    match (&ssl_directives, force_https) {
        (Some(sd), true) => {
            ...
            out.push_str(&render_security(security, super::waf::waf_ready()));
...
mod tests {
    ...
    fn security_off_renders_nothing() {
        ...
        assert_eq!(render_security(Some(&sec), true), "");
        assert_eq!(render_security(None, true), "");
    }

    /// 限速 / 限并发：按配置渲染，超额统一回 429
    #[test]
    fn security_renders_limits() {
        let sec = SiteSecuritySpec {
            limit_req_enable: true,
            limit_req_rate: 10,
            limit_req_burst: 20,
            limit_conn_enable: true,
            limit_conn_num: 50,
            ..Default::default()
        };
        let s = render_security(Some(&sec), false);
        assert!(s.contains("limit_req zone=zap_req_limit rate=10r/s burst=20 nodelay;"), "{s}");
        assert!(s.contains("limit_req_status 429;"), "{s}");
        assert!(s.contains("limit_conn zap_conn_limit 50;"), "{s}");
        // 全局 WAF 不可用时不该写出 modsecurity 指令
    ...
    fn security_waf_requires_global_ready() {
        ...
        assert!(render_security(Some(&sec), true).contains("modsecurity on;"));
        assert!(!render_security(Some(&sec), false).contains("modsecurity"));
    }

    /// 0 值兜底：rate / burst / conn 为 0 时 Nginx 不接受，渲染前抬到 1
    #[test]
    fn security_clamps_zero_values() {
        let sec = SiteSecuritySpec {
            limit_req_enable: true,
            limit_req_rate: 0,
            limit_req_burst: 0,
        ...
        let s = render_security(Some(&sec), false);
        assert!(s.contains("rate=1r/s"), "{s}");
        assert!(s.contains("burst=1"), "{s}");
        assert!(s.contains("limit_conn zap_conn_limit 1;"), "{s}");
    }
