...
//! - 状态页监控：探测 / 启停 `stub_status` 并采集请求统计（托管于本机 127.0.0.1）
...
/// Zap 托管的状态页 server 文件名（sites-available 下，zap- 前缀 → 编辑保护）。
const STUB_SERVER_FILE: &str = "zap-stub.conf";
/// 状态页 URL 路径。
...
/// Zap 托管状态页配置文件路径。
fn stub_conf_path() -> PathBuf {
    super::webconf::available_dir("nginx").join(STUB_SERVER_FILE)
}

/// 渲染状态页 server（仅监听 127.0.0.1，拒绝外部访问）。
fn render_stub_vhost(port: u16) -> String {
    format!(
        "# Generated by Zap Panel — Nginx 状态页 stub_status（本机采集）— DO NOT EDIT\n\
         server {{\n\
...
/// 从已发布配置解析状态页监听端口；未启用返回 None。
fn stub_port_from_conf() -> Option<u16> {
    let content = std::fs::read_to_string(stub_conf_path()).ok()?;
    content.lines().find_map(|line| {
        let l = line.trim();
        let rest = l.strip_prefix("listen 127.0.0.1:")?;
...
pub async fn stub_status(enable: Option<bool>) -> Response {
    tokio::task::spawn_blocking(move || -> Result<Response, String> {
        ...
        match enable {
            None => {
                let enabled = stub_conf_path().exists();
                let port = stub_port_from_conf();
                let metrics = if enabled && running {
                    match port.and_then(|p| fetch_stub(p).ok()) {
                        Some(m) => json!({
                            "active": m.active,
                            "accepts": m.accepts,
                            "handled": m.handled,
            ...
            Some(on) => {
                ...
                let new_content = new_port.map(render_stub_vhost);
                let rollback = || {
                    match &new_content {
                        Some(_) => match &old {
                            Some(o) => {
                ...
                if let Some(content) = &new_content {
                    if let Err(e) =
                        super::webconf::publish_named("nginx", STUB_SERVER_FILE, content)
                    {
                        rollback();
                        return Err(format!("写入状态页配置失败: {e}"));
                    }
                } else if let Err(e) = super::webconf::purge_named("nginx", STUB_SERVER_FILE) {
                    rollback();
                    return Err(format!("移除状态页配置失败: {e}"));
                }
                if let Err(e) = site::nginx_test(&bin) {
                    rollback();
